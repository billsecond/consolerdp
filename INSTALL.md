...
- `~/.local/share/krdpserver/krdp.{crt,key}` — self-signed RSA-2048 TLS cert for NLA, generated by `openssl` on first install (10-year validity).
- `~/.config/krdpserverrc` — `Autostart=true`, `SystemUserEnabled=true` so RDP auth uses the user's Linux password via PAM (no separate KCM step needed).
- `loginctl enable-linger <user>` — keeps the user's systemd manager running across reboots so `app-org.kde.krdpserver.service` starts at boot.
- `app-org.kde.krdpserver.service` — enabled and started for the seat user.
